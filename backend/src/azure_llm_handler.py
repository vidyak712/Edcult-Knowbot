...
AZURE_OPENAI_EMBED_API_VERSION = os.getenv("AZURE_OPENAI_EMBED_API_VERSION")
...
embeddings = AzureOpenAIEmbeddings(
    ...
    api_version=AZURE_OPENAI_EMBED_API_VERSION,
    azure_endpoint=AZURE_OPENAI_EMBED_ENDPOINT,
    model=AZURE_OPENAI_EMBED_MODEL_NAME
)
...
def search_azure_index(query, top=10):
    """
    Search Azure Search Index using hybrid search (vector + keyword).
    """
    headers = {
...
def call_azure_openai(query, documents, history=None):
    ...
    if request_table:
        ...
        - Only include columns that actually exist in the source documents — do NOT add, infer, or split columns
        - Do NOT separate units into their own column unless the source document has a Units column
        - Keep cell values exactly as they appear in the source — do not paraphrase or reformat values
        - If the same parameter appears with different values across pages, include both values in one cell with page references (e.g. "600 litres (p.6) / 4500 litres (p.11)")
        - Do NOT invent rows that are not present in the retrieved documents

        ANSWER STRUCTURE:
        - Lead with the table directly — no preamble.
        - After the table, note any conflicts or ambiguities found across source pages.
        ...
        Reproduce the table exactly as it appears in the source documents. Use only the columns present in the source. Do not add a Units column unless one exists in the source."""
    else:
        system_prompt = """You are a helpful assistant that answers questions based on provided document content. 
        Answer the user's question using only the information from the provided documents.
...
def generate_response_from_query_with_history(request: LLMRequest):
    """
    ...
    search_results = search_azure_index(query, top=10)
